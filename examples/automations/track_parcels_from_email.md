# Automatic parcel tracking from e-mail (IMAP → Canpar)

Companion guide for [`track_parcels_from_email.yaml`](track_parcels_from_email.yaml): watch your mailbox(es) for shipping e-mails, extract the Canpar tracking code, and register it with `canpar.track_parcel` — fully automatic, no extra custom component required.

Canpar is a **code-based** carrier: it has no account inbox, so every parcel must be registered by its tracking code before the integration can follow it. This recipe automates exactly that step.

**How it works, in one line:** the core [IMAP integration](https://www.home-assistant.io/integrations/imap/) fires an `imap_content` event for every new e-mail (including the body); the automation extracts the tracking code — a cheap regex first, an optional AI fallback for everything else — and calls `canpar.track_parcel`.

```
new e-mail ──imap_content──▶ automation ──▶ regex match? ──▶ canpar.track_parcel
                                     │
...
- This integration, with the `canpar.track_parcel` action available (field `tracking_code`).
...
The regex matches the confirmed Canpar family: `D` followed by 20 digits.

```
(?<![0-9A-Za-z])D[0-9]{20}(?![0-9A-Za-z])
```

The integration accepts any plausible `6–30`-character alphanumeric code, because other Canpar families are still unconfirmed. Matching that broadly in e-mail would hit ordinary order numbers, so this example intentionally extracts only the observed `D` + 20-digit family. If you receive a different confirmed family, add it to the alternation; the AI fallback can handle unfamiliar formats in the meantime.
...
1. **Jinja eats backslashes in string literals.** A template stored as `regex_findall('\bD…')` silently becomes a **backspace character** (`\b` is a string escape), so the regex never matches — no error anywhere. That's why the pattern is backslash-free: `(?<![0-9A-Za-z])` lookarounds instead of `\b`, `[0-9]` instead of `\d`. Copy that style if you add another family.
...
curl -X POST -H "Authorization: Bearer $HA_TOKEN" -H "Content-Type: application/json" \
  ...
  -d '{"sender":"noreply@canparshipping.com","subject":"Your parcel is on its way",
       "text":"Tracking code: D000000000000000000001","initial":true,"folder":"INBOX","username":"test"}'
```

Then `canpar.untrack_parcel` the test code afterwards. For a full end-to-end test, forward a real shipping mail to the watched mailbox — it must arrive **unread**.
