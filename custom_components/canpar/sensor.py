"""Sensor platform for the Canpar parcel tracker integration."""
...
from .coordinator import CanparCoordinator
...
async def async_setup_entry(
    ...
    """Set up Canpar sensor entities from a config entry."""
    ...
    entities: list[SensorEntity] = [
        CanparIncomingParcelsSensor(
            coordinator, entry, async_add_entities, current_barcodes
        ),
        CanparAwaitingPickupSensor(coordinator, entry),
    ]
    for parcel in coordinator.data or []:
        entities.append(
            CanparParcelSensor(coordinator, entry, parcel.get("barcode", ""))
        )
    entities.append(CanparNextDeliverySensor(coordinator, entry))
    entities.append(CanparDeliveredParcelsSensor(coordinator, entry))
    entities.append(CanparLastUpdateSensor(coordinator, entry))

    async_add_entities(entities)


class CanparAwaitingPickupSensor(
    CoordinatorEntity[CanparCoordinator], SensorEntity
):
    ...
    def __init__(self, coordinator: CanparCoordinator, entry: ConfigEntry) -> None:
...
class CanparIncomingParcelsSensor(
    CoordinatorEntity[CanparCoordinator], SensorEntity
):
    ...
    def _handle_coordinator_update(self) -> None:
        ...
        if new_barcodes:
            self._async_add_entities(
                CanparParcelSensor(self.coordinator, self._entry, barcode)
...
class CanparParcelSensor(CoordinatorEntity[CanparCoordinator], SensorEntity):
    """Per-parcel sensor reporting the status of one tracked Canpar parcel."""
    ...
    def __init__(
        self, coordinator: CanparCoordinator, entry: ConfigEntry, barcode: str
    ) -> None:
        """Initialize the sensor."""
        super().__init__(coordinator)
        self._entry = entry
...
class CanparNextDeliverySensor(
    CoordinatorEntity[CanparCoordinator], SensorEntity
):
    ...
    def __init__(
        self, coordinator: CanparCoordinator, entry: ConfigEntry
    ) -> None:
        """Initialize the sensor."""
        super().__init__(coordinator)
...
class CanparDeliveredParcelsSensor(
    CoordinatorEntity[CanparCoordinator], SensorEntity
):
    """Recently delivered tracked Canpar parcels."""
...
class CanparLastUpdateSensor(
    CoordinatorEntity[CanparCoordinator], SensorEntity
):
    """Diagnostic sensor reporting when Canpar was last polled successfully."""
