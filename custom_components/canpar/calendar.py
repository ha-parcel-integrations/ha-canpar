"""Calendar platform for the Canpar parcel tracker integration."""
...
async def async_setup_entry(
    ...
    """Set up the Canpar deliveries calendar from a config entry."""
    async_add_entities([CanparDeliveriesCalendar(entry.runtime_data.coordinator, entry)])


class CanparDeliveriesCalendar(CoordinatorEntity[CanparCoordinator], CalendarEntity):
    """A read-only calendar of expected Canpar deliveries.
    ...
    def _events(self) -> list[CalendarEvent]:
        ...
        for parcel in self.coordinator.data or []:
            ...
            summary = sender or (f"Parcel {barcode}" if barcode else "Canpar parcel")
            description_parts = [
