"""Coordinator for the Canpar parcel tracker integration.
...
from .api import CanparApiClient, CanparApiError
from .const import (
...
class CanparCoordinator(DataUpdateCoordinator[list[dict]]):
    ...
    async def _async_update_data(self) -> list[dict]:
        ...
        for code, result in zip(codes, results):
            if isinstance(result, BaseException):
                if not isinstance(
                    result, (CanparApiError, aiohttp.ClientError)
                ):
                    raise result
                errors += 1
                if isinstance(result, CanparApiError) and result.status_code == 429:
                    saw_429 = True
                    if result.retry_after is not None:
                        retry_afters.append(result.retry_after)
                _LOGGER.warning("Canpar fetch failed for %s: %s", code, result)
        ...
        if saw_429:
            ...
            raise UpdateFailed("Canpar rate-limited (429)", retry_after=retry_after)
        self._consecutive_429 = 0

        if codes and errors == len(codes) and not raws:
            raise UpdateFailed("Canpar unreachable for all tracked parcels")
