"""Canpar parcel tracker custom component for Home Assistant."""
...
from .api import CanparApiClient
...
@dataclass
class CanparData:
    """Runtime data attached to the Canpar config entry."""

    client: CanparApiClient
    coordinator: CanparCoordinator


type CanparConfigEntry = ConfigEntry[CanparData]


async def async_setup_entry(hass: HomeAssistant, entry: CanparConfigEntry) -> bool:
    """Set up Canpar from a config entry."""
    # No auth: Canpar tracking is public, so the HA-managed session is fine.
    client = CanparApiClient(async_get_clientsession(hass))
    coordinator = CanparCoordinator(hass, client, entry)
    ...
    entry.runtime_data = CanparData(client=client, coordinator=coordinator)
...
async def async_unload_entry(hass: HomeAssistant, entry: CanparConfigEntry) -> bool:
    """Unload the Canpar config entry."""
