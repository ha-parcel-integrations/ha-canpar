"""Tests for the Canpar coordinator: fetching, caching and events.
...
from custom_components.canpar.api import CanparApiError
...
from custom_components.canpar.coordinator import CanparCoordinator

from .payloads import ACTIVE_CODE, DELIVERED_CODE, active_sample, delivered_sample

OTHER_CODE = "D000000000000000000003"
...
def _in_transit(code: str = ACTIVE_CODE) -> dict:
    ...
    sample["events"][0]["code"] = "SRT"
    sample["events"][0]["code_description_en"] = "Sorting"
    return sample
...
async def test_update_keeps_cached_payload_on_error(hass):
    ...
    client.async_get_parcel.side_effect = CanparApiError("HTTP 500")
...
async def test_does_not_fire_delivery_time_changed_event_without_a_verified_window(hass):
    entry = _entry_with([{CONF_TRACKING_CODE: ACTIVE_CODE}])
    entry.add_to_hass(hass)
    client = AsyncMock()
    coordinator = CanparCoordinator(hass, client, entry)
